...

{
  ...
  if (threadIdx.x == 0) {
    ...
    if (problem.dimensions_info.has_dimension(dim_t::SOFT_TIME)) {
      auto& ls                         = route.template get_dim<dim_t::SOFT_TIME>();
      ...
      ls.due_time[0]                   = 0;
      ls.order_weight[n_nodes_route]   = 0;
      ls.node_info[n_nodes_route]      = NodeInfo<>{};  // default = DEPOT type
      ls.earliest_time[n_nodes_route]  = 0;
      ls.due_time[n_nodes_route]       = 0;
      // Lateness objective: zero prefix sum at depot; return depot has empty backward array.
      ls.fwd_lateness[0]               = 0;
      ls.bwd_lateness_n[n_nodes_route] = 0;
    }
